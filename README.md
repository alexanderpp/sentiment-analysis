...
After playing around with the algorithms, one of the will be chosen as the best one. It will be trained and tweaked for the best results.
...
The data used for testing and training is part Keggle competition called "Bag of Words Meets Bags of Popcorn".
...
For all of the data is split into training data and test data, using the following proportions - 70% for training and 30% for testing.
...
It contains 25,000 equally split samples for training and another 25,000 (again, equally split), for testing.
...
On every position of the vector (corresponding to the index of word) is the count of times this word is found in the current sample text.
...
Word2vec is a neural net which processes text.
The input of this net is a set of text examples and the output is a set of vectors, where every word is represented as a multidimensional vector.
...
It is named Bayes, because it is based on and applies Bayes' theorem which looks like this:
...
If we apply this theorem to an actual problem, it will look like this:
...
In the Bayes Classifier, we can compute the most probable classification of new example by combining the predictions of all the hypothesis, weighed by their ** *a posteriori* probabilities**

The Naive Bayes classifier is a method for Bayes learning, which has proven its usability in numerous tasks.
...
In order to achieve that they create a big network of so called ```perceptrons``` - specially designed mathematical models that simulate biological neurons.

Basically a perceptron looks like this:
...
* Weights - weights are the metric that describes how meaningful is each input for the final classification.
...
To get better idea of the situation, check the picture below. Note that the links between the input and the output are replaced by X for simplicity.
...
Before talking about LSTM it is only natural that we first mention something about Recurrent Neural Networks in general.

A recurrent neural network is very similar to normal neural network.
In fact if looked from the outmost perspective it looks exactly the same - input layer, some hidden layers and an output layer.
However it is very different in the way the hidden layers behave.
...
This basically means that Recurrent Neural Networks are aware of their previous state.
You can also say that they remember a context, or that they are context aware.
...
LSTM is basically a Recurrent Neural Network that uses several steps of logic gates, which controls the flow of input through it.
...
* At first it forgets the useless (irrelevant) parts of the previous state;
...
* Output certain parts of the cell state that are considered relevant.

This way all the irrelevant information is forgotten to give priority to remembering only what is relevant.
...
As you can see, the results using this datasets are a little bit lower for most of the algorithms (for all without LSTM).
There are two reasons for this to happen:
* With this dataset we have more data to train, but also a lot more data to test with. Therefore it is normal that the results will be a little lower
* All the algorithms except for LSTM were tweaked to work good with the first dataset. A little bit of tweaking the parameters can lead to better results with the second dataset, but this is not the scope of the current project.

### Let's take a further look at the LSTM scoring the best results
...
* LSTM layer
* Dropout layer - Dropout is a technique which aims to reduce the overfitting in a Neural network. Its tries to prevent the the adaptation to the training data.
* Dense layer - The final layer is a simple dense layer which presents the output as a single value.

#### Learning process

The learning process using LSTM is slow.
It becomes even slower, when we add the transformation of every single word into vectors (using word2vec).
...
Each gave their results and finally, it was concluded that from the chosen algorithms it was LSTM that solved the problem the best way.
That is not a surprise, because a lot of research lead to the fact that Recurrent Neural Networks are the best at solving problems of this matter.
LSTM is a concept that has gone even further and provided a brilliant solution.
